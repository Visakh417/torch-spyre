...
def test_find_shifted_kernel_timestamps():
    """Verify shifted-kernel detection with synthetic trace events."""
    # Normal pair: kernel starts after CPU dispatch
    ok_events = [
        {
            "ph": "X",
    ...
    # Shifted pair: kernel ts is earlier than CPU dispatch ts
    shifted_events = [
        {
            "ph": "X",
            "cat": "privateuse1_runtime",
    ...
    # Events without a correlation key — must be silently skipped
    no_corr_events = [
        {
            "ph": "X",
            "cat": "privateuse1_runtime",
    ...
    ok_pairs, ok_shifted = _find_shifted_kernel_timestamps(ok_events)
    assert len(ok_pairs) == 1
    assert ok_shifted == []

    shifted_pairs, shifted_shifted = _find_shifted_kernel_timestamps(shifted_events)
    assert len(shifted_pairs) == 1
    assert len(shifted_shifted) == 1
    cpu_ev, k_ev = shifted_shifted[0]
    assert k_ev["ts"] < cpu_ev["ts"]

    no_corr_pairs, no_corr_shifted = _find_shifted_kernel_timestamps(no_corr_events)
    assert no_corr_pairs == []
    assert no_corr_shifted == []
